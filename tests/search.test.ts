...
test("search params", async () => {
  ...
  // single literal
  expect(await search({ bin, folder: "./", literal: "123" })).toEqual([]);
  expect(cmd).toEqual(["echo", "-F", "123", "./"]);

  // multi literal
  expect(await search({ bin, folder: "./", literal: ["123", "456"] })).toEqual(
    [],
  );
  expect(cmd).toEqual(["echo", "-F", "123", "-F", "456", "./"]);

  // single regex
  expect(await search({ bin, folder: "./", regex: "123" })).toEqual([]);
  expect(cmd).toEqual(["echo", "-e", "123", "./"]);

  // multi regex
  expect(await search({ bin, folder: "./", regex: ["123", "456"] })).toEqual(
    [],
  );
  ...
  // single globs
  expect(
    await search({ bin, folder: "./", globs: "123", literal: "123" }),
  ).toEqual([]);
  ...
  // multi folder
  expect(
    await search({ bin, folder: ["./", "./foo"], literal: "123" }),
  ).toEqual([]);
  expect(cmd).toEqual(["echo", "-F", "123", "./", "./foo"]);
});
